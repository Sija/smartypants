...
class SmartyPantsParser
{
    ...
    #
    public static $DEFAULT_ATTR = 1;
    
    # SmartyPants will not alter the content of these tags:
    public static $TAGS_TO_SKIP = 'pre|code|kbd|script|style|math';
    
    # Options to specify which transformations to make:
    protected $do_quotes    = 0;
    protected $do_backticks = 0;
    protected $do_dashes    = 0;
    ...
    #
        $attr = !empty($attr) ? $attr : self::$DEFAULT_ATTR;
        if ($attr === 1) {
    ...
    public function transform($text)
    {
        ...
        foreach ($tokens as $cur_token) {
            if ($cur_token[0] == 'tag') {
                ...
                $result .= $cur_token[1];
                if (preg_match('@<(/?)(?:'.self::$TAGS_TO_SKIP.')[\s>]@', $cur_token[1], $matches)) {
